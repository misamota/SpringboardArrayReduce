...
lwrStr = str.toLowerCase().split("");
vowels = "aeiou"
return lwrStr.reduce(function(ob,let){
     if(vowels.includes(let)){
         if(ob[let]){
            ob[let] += 1;
         }else{
            ob[let] = 1;
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function(newArr,val,idx){
...
Write a function called partition which accepts an array and a callback and returns an array with
 ...
 in the array and if the result of the callback function at that specific value is true,
  the value should be placed in the first subarray. If the result of the callback function a
  t that specific value is false, the value should be placed in the second subarray. 
...
return arr.reduce((doubleArr,v)=>{
 callback(v) ? doubleArr[0].push(v) : doubleArr[1].push(v);
 return doubleArr;
},[[],[]]);
};
